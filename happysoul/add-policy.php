...
<?php 
    ...
    // if(isset($_POST['btnsave']) && isset($_POST['policyid'])){
    //     $policyid  = $_POST['policyid']; 

    //     // $model = $_POST['model'];
    //     $itemname = $_POST['itemname'];
       
    //     $query = "update tblpolicy set sName=? where iPolicyId  = ?";
    //     $stmt = mysqli_prepare($link,$query);
    //     mysqli_stmt_bind_param($stmt, "si", $itemname,$policyid);
    ...
    //         echo "<script>window.location.href = 'list-policy.php';</script>";
    //     } 
    ...
    if(isset($_POST['btnsave']))
    {
        // $model = $_POST['model'];
          ...
          if(isset($_FILES['pdffile']) && $_FILES['pdffile']['size'] > 0){
            ...
            $time = time();
            $target_file = $target_dir ."policypdf_". $time.".".$ext;       
            $imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
            
            if (move_uploaded_file($_FILES["pdffile"]["tmp_name"], $target_file)) {
                $imagepath = server_path."policypdfs/policypdf_". $time.".".$ext;
            }
        }

            $query = "INSERT INTO tblpolicy (sText,sPDFUrl,iAddedBy) VALUES (?,?,?)";
            $stmt = mysqli_prepare($link,$query);
            ...
            if(!$ret){
                ...
                $message  = "{'type':'New Policy','dataid':'0','text':'".$msg."'}";
    ...
    else if(isset($_POST['policyid'])){
        $policyid  = $_POST['policyid']; 
            
        $stmt = $link->prepare('select * from tblpolicy where iPolicyId  = ?');
                    ...
                    <div class="col-xl-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <form action="add-policy.php" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                 <!--         
                                    <div class="col-md-6">
                                            <div class="mb-3">
